...
async def gopro_controller(args: argparse.Namespace, output_dir: Path):
    ...
    while True:
        try:
                ...
                while True:
                    if take_photos:
                        ...
                        if not media_list_before.ok:
                            ...
                            continue
                        media_set_before = set(media_list_before.data.files)
                        ...
                        for _ in range(5):  # Retry 5 times
                            ...
                            if media_list_after.ok:
                                new_photos = set(media_list_after.data.files).difference(media_set_before)
                                if new_photos:
                                    new_photo = new_photos.pop()
                                    break
                        ...
                        output_file = output_dir / f"{timestamp}_{new_photo.filename}"
                        console.print(f"Downloading {new_photo.filename}...")
                        await gopro.http_command.download_file(
                            camera_file=new_photo.filename, local_file=output_file
                        )
            ...
            console.print(f"[bold red]GoPro Error: {e}. Retrying in 10 seconds...[/bold red]")
            gopro_is_ready = False
            if gopro:
                await gopro.close()
...
async def mavlink_listener(connection_string: str):
    ...
    while True:
        try:
            ...
            while True:
                # Check for a STATUSTEXT message
                msg = master.recv_match(type="STATUSTEXT", blocking=False)
                if msg:
                    message_text = msg.text.strip()
                    # Check for the specific camera trigger command
